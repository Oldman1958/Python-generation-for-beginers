...
Требуется написать программу, которая находит стоимость старого и нового сообщений Сэма в 🐝
...
letter_rus = 'еуорахсЕТОРАНХСВМ'
cash_old = 0
cash_new = 0
for i in range(len(com_in)):
    if com_in[i] in letter_eng:
        cash_new += ord(letter_rus[letter_eng.index(com_in[i])]) * 3
    else:
        cash_new += ord(com_in[i]) * 3
    cash_old += ord(com_in[i]) * 3

print(f"Старая стоимость: {cash_old}🐝")
print(f"Новая стоимость: {cash_new}🐝")
